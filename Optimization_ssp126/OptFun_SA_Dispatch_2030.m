...
%% ======================== 2. 扣除基荷发电 ========================
% AR6 SSP1-2.6 情景下，2030年基荷发电占73.7%
grid_load=loads;
for gg_ind=1:20
    tmp=grid_load(gg_ind,:);
    tmp=tmp-sum(tmp)*0.737/8760;  % 基荷占比73.7%
    grid_load(gg_ind,:)=tmp;
end

%% ======================== 3. 储能与输电参数 ========================
toStorageLoss=0.95;
fromStorageLoss=0.95;
load Global_Trans trans_connections trans_loss
trans_connections(trans_connections==2)=0;  % 移除跨洲连接

storagePow=scale(length(CGrid_Index)+1:length(CGrid_Index)+20)/1000;
storageCap=storagePow.*scale(length(CGrid_Index)+21:length(CGrid_Index)+40);
...
obj_cost=obj_cost+136.07*sum(ins_cap(index(index>nonlsol)));
