%% Optimization_SC_2050.m — 2050年 SSP1-2.6 情景下的空间布局优化（大陆互联 S-C）
%
...
%% ======================== 8. 运行 NSGA-II 优化 ========================
% AR6 校准参数（由 binary_search_driver.m 设置，手动运行时使用默认值）
if ~exist('solar_mult','var'), solar_mult = 1.0; end
if ~exist('cap_penalty','var'), cap_penalty = 0; end

T = datetime('now');
disp(T)
fprintf('开始2050年优化（大陆互联，种群=500，代数=200）\n');
fprintf('AR6 校准参数：solar_mult=%.3f, cap_penalty=%.2f\n', solar_mult, cap_penalty);
options = optimoptions('gamultiobj','UseParallel',true,'PlotFcn',[],'PopulationSize',500,'MaxGenerations',200);
[res_scale,prs]=gamultiobj(@(scale)  OptFun_SC_Dispatch_2050(all_ins,all_gens,all_loads,CGrid_Index,scale,solar_mult,cap_penalty),...
...
%% ======================== 9. 保存结果 ========================
% 文件名包含校准参数以便追踪
out_suffix = sprintf('_sm%.2f_cp%.1f', solar_mult, cap_penalty);
out_file = ['Optimization_SC_2050_Res', out_suffix, '.h5'];
if exist(out_file,'file'), delete(out_file); end
h5create(out_file,'/res_scale',size(res_scale));
h5write(out_file,'/res_scale',res_scale);
h5create(out_file,'/prs',size(prs));
h5write(out_file,'/prs',prs);
fprintf('结果已保存至 %s\n', out_file);
