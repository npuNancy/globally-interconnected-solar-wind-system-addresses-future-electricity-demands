function f = OptFun_SC_Dispatch_2050(ins_cap,gens,loads,CGrid_Index,scale,solar_mult,cap_penalty)
...
%   solar_mult  - 光伏成本倍率（1.0=原始，>1 提高光伏成本以调整光风比）
%   cap_penalty - 容量惩罚系数（$B/GW，惩罚超出 AR6 目标的总装机）
%
% 输出：
...
obj_cost=obj_cost+927.6*solar_mult*sum(ins_cap(index(index<=nonlsol)));   % 光伏（×solar_mult）
obj_cost=obj_cost+1313*sum(ins_cap(index(index>nonlsol)));     % 陆上风电

% 北美（区域1）
index=find((CGrid_Index(:,1)==1)&(CGrid_Index(:,2)==1)&(CGrid_Index(:,3)==0));
obj_cost=obj_cost+1012.6*solar_mult*sum(ins_cap(index(index<=nonlsol)));  % 光伏（×solar_mult）
...
obj_cost=obj_cost+1075.9*solar_mult*sum(ins_cap(index(index<=nonlsol)));  % 光伏（×solar_mult）
...
obj_cost=obj_cost+861.4*solar_mult*sum(ins_cap(index(index<=nonlsol)));   % 光伏（×solar_mult）
...
obj_cost=obj_cost+1256.6*solar_mult*sum(ins_cap(index(index<=nonlsol)));  % 光伏（×solar_mult）
...
obj_cost=obj_cost+922.5*solar_mult*sum(ins_cap(index(index<=nonlsol)));   % 光伏（×solar_mult）
...
% 容量惩罚：惩罚总装机超出 AR6 目标（12217 GW）的部分
AR6_TARGET_GW = 12217;
total_cap_GW = sum(ins_cap(CGrid_Index(:,2)==1)) * 1000;  % TWp → GW
f(3) = obj_cost + cap_penalty * max(0, total_cap_GW - AR6_TARGET_GW);  % 总成本 + 容量惩罚
end
