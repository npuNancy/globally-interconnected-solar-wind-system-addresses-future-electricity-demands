...
for ii=1:20
    ii
    tmpindex=find(CGrid_Index(:,1)==ii);
    scale=res_scale(66,:);
    scale(tmpindex)=0;
    trans=trans_connections;trans(trans>0)=1;
    [f,con,flex,curtail,storage,shifted]=Fun_Dispatch_2050_Sen(all_ins,all_gens,all_loads,CGrid_Index,scale,trans,6);
    res_supply(ii,1)=1-f(2)-0.09;
    [f,con,flex,curtail,storage,shifted]=Fun_Dispatch_2050_Sen(all_ins,all_gens,all_loads,CGrid_Index,scale,trans,3);
    res_supply(ii,2)=1-f(2)-0.09;
    [f,con,flex,curtail,storage,shifted]=Fun_Dispatch_2050_Sen(all_ins,all_gens,all_loads,CGrid_Index,scale,trans,2);
    res_supply(ii,3)=1-f(2)-0.09;
    scale(length(CGrid_Index)+41:end)=0;
    [f,con,flex,curtail,storage,shifted]=Fun_Dispatch_2050_Sen(all_ins,all_gens,all_loads,CGrid_Index,scale,trans,2);
    res_supply(ii,4)=1-f(2)-0.09;
end
% ins_cap=all_ins;
% gens=all_gens;
% loads=all_loads;
...
res_supply=res_supply-(1-f(2)-0.09);
res_supply=res_supply*100;
figure,boxplot(res_supply)
...
%     trans_power(trans_connections>0)=scale(length(CGrid_Index)+41:end)/1000; %transmission power TW
%     trans=trans_connections;trans(trans==2)=0;
%     tmp=trans_power(trans==1);
%     scale(length(CGrid_Index)+41:end)=[];
%     scale=[scale(:);tmp(:)];
%     [f,con,flex,curtail,storage,shifted]=Fun_Dispatch_2050_Sen(all_ins,all_gens,all_loads,CGrid_Index,scale,trans,3);
%     res_supply(ii,2)=f(1);
%     [f,con,flex,curtail,storage,shifted]=Fun_Dispatch_2050_Sen(all_ins,all_gens,all_loads,CGrid_Index,scale,trans,2);
