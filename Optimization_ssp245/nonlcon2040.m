...
%
% 用法：
%   nonlcon2040(x)                          — 由 ga 调用，从持久变量加载配置
%   nonlcon2040(x, cost_cfg, scenario_cfg, model_data, persistent_data)
%                                           — 直接传参（用于测试）
%
% 注意：单参数模式需要先运行 Optimization_SC_2040.m 保存模型数据
%       至 results/model_data_2040.mat。
    ...
    if nargin == 1
        ...
        if isempty(p_cost_cfg)
            ...
            data = load('results/model_data_2040.mat', 'model_data', 'persistent_data');
