"""
run_optimization_SA_2030.py — 2030年相邻互联情景（S-A）空间布局优化主程序

对应 MATLAB Optimization_SA_2030.m。
从 2040 (S-C) 最优解中筛选候选格网，使用 NSGA-II 求解帕累托最优解集。

用法:
    cd /data6/yanxiaokai/project_energy_climate/globally_interconnected_reRun
    source .venv/bin/activate
    python -m Optimization_py.run_optimization_SA_2030
"""
...
from Optimization_py.config import SCENARIO_SA_2030
...
def main():
    """主优化流程"""
    scenario = SCENARIO_SA_2030
    pop_size = 200  # 种群大小
    n_gen = 100  # 最大代数

    print(f"========== 2030 相邻互联 (S-A) 空间布局优化 ==========")
    ...
    if result.X is not None and len(result.X) > 0:
        output_file = os.path.join(output_dir, "Optimization_SA_2030_Res_reRun.h5")
