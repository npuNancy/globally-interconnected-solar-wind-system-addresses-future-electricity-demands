"""
run_optimization_SC_2040.py — 2040年洲内互联情景（S-C）空间布局优化主程序

对应 MATLAB Optimization_SC_2040.m。
从 2050 (S-G) 最优解中筛选候选格网，使用 NSGA-II 求解帕累托最优解集。

用法:
    cd /data6/yanxiaokai/project_energy_climate/globally_interconnected_reRun
    source .venv/bin/activate
    python -m Optimization_py.run_optimization_SC_2040
"""
...
from Optimization_py.config import SCENARIO_SC_2040
from Optimization_py.data_loader import (
    load_wind_data,
    load_solar_data,
    load_load_profiles,
    load_transmission_data,
    load_previous_optimal,
    build_combined_data,
    build_variable_bounds_from_previous,
)
...
def main():
    """主优化流程"""
    scenario = SCENARIO_SC_2040
    pop_size = 200  # 种群大小
    n_gen = 100  # 最大代数

    print(f"========== 2040 洲内互联 (S-C) 空间布局优化 ==========")
    ...
    # ==================================================================
    # 阶段零：加载前序情景（2050 S-G）的最优解
    # ==================================================================
    print("\n--- 阶段零：加载前序情景最优解 ---")
    previous = load_previous_optimal(opt_dir, scenario["previous_optimal_file"])
    print(f"  前序情景: {scenario['previous_optimal_file']}")
    print(f"  光伏格网: {int(previous['opt_solar'].sum())} 个被选中")
    print(f"  风电格网: {int(previous['opt_wind'].sum())} 个被选中")
    ...
    # 加载风电数据（使用前序最优筛选）
    wind = load_wind_data(opt_dir, previous_optimal=previous)

    # 加载光伏数据（使用前序最优筛选）
    solar = load_solar_data(opt_dir, previous_optimal=previous)
    ...
    # 从前序最优构建变量边界
    bounds = build_variable_bounds_from_previous(combined, opt_dir, previous, scenario)
    ...
    if result.X is not None and len(result.X) > 0:
        output_file = os.path.join(output_dir, "Optimization_SC_2040_Res_reRun.h5")
