%% Optimization_SC_2040.m — 2040年 SSP1-2.6 情景下的空间布局优化（大陆互联 S-C）
%
% 功能：使用 NSGA-II（gamultiobj）多目标优化，求解2040年全球20个区域的光伏/风电场站
...
% 预筛选（presel）：仅保留2050年有效开发的风电格网（比例 > EPS_ACTIVE）
win_index2 = find(opt_wind > cfg.EPS_ACTIVE);
wind_ub = opt_wind(win_index2);  % 2050年开发比例作为上限
...
clear pv_density
% 预筛选（presel）：仅保留2050年有效开发的光伏格网（比例 > EPS_ACTIVE）
solar_index2 = find(opt_solar > cfg.EPS_ACTIVE);
solar_ub = opt_solar(solar_index2);  % 2050年开发比例作为上限
...
%% ======================== 6. 加载2040年负荷数据 ========================
% SSP1-2.6 对应第3个情景维度（索引=3）
load Global_Load_22region.mat
all_loads=squeeze(all_loads(3,:,:))/1000/1000;%单位 TW
global_load=sum(all_loads,1);
% 缩放至 AR6 SSP1-2.6 的 44,860 TWh
all_loads=all_loads/(sum(global_load(:))/44860);
clear global_load
...
nonlcon_ub=[solar_ub(:); wind_ub(:)];  % 2040年格网比例上限 = 2050年开发比例
...
ub(n_grid+41:n_grid+40+sum(tmp(:)))=opt_trans(tmp);% 上界=2050年输电配置（GW）
clear cur_trans
nvars=length(lb);
...
load NonlConData2040.mat nonlcon_sel nonlcon_ins nonlcon_ub nonlsol nonlwin
...
T = datetime('now');
disp(T)
fprintf('开始2040年连续容量优化（大陆互联，种群=%d，代数=%d）...\n', ...
